...
NOTE: 如果目标集群上不存在 AppVault CR，请按照 link:trident-protect-appvault-custom-resources.html["使用 Trident Protect AppVault 对象管理存储桶"] 中的步骤创建它。
...
+-------------+-----------+--------+-----------------+--------------------------+-------------+
----
. 使用 AppVault 名称和存档路径将应用程序还原到目标集群：注意：使用 CR 时，请确保目标集群上存在用于应用程序还原的命名空间。
