...
选择适合您的安装方法。您还应该在做出决定之前查看 link:kubernetes-deploy.html#moving-between-installation-methods["在方法之间移动"] 的注意事项。



=== 使用 Trident 操作员

无论是手动部署还是使用 Helm，Trident 操作员都是简化安装和动态管理 Trident 资源的好方法。您甚至可以link:../trident-install/kubernetes-customize-deploy.html["自定义您的 Trident 操作员部署"]使用 `TridentOrchestrator`自定义资源 (CR) 中的属性。
